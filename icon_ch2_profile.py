...
FILE_HASH = "ultimo_hash_icon_ch2.txt"
...
def verifica_dati_nuovi(hourly_data: dict) -> bool:
    ...
    # ICON-CH2 a 5 giorni deve avere circa 120 ore. Usiamo 115 come margine di sicurezza.
    if len(valid_mean) < 115 or len(valid_spread) < 115:
        print(f"⏳ Dati insufficienti (ore valide: {len(valid_mean)}/120). Attendo run...")
        return False
    ...
    if mean_cambiata and spread_cambiato:
        ...
        if mean_cambiata or spread_cambiato:
            print("⏳ Aggiornamento API ICON-CH2 a metà (Media o Spread non allineati). Attendo...")
        return False
...
def fetch_dati_con_retry() -> dict:
    ...
    headers = {"User-Agent": "MeteoBot-ICONCH2/1.0"}
...
def main():
    print("Scaricamento dati ICON-CH2 Ensemble Mean (5 Giorni)...")
    ...
    if not verifica_dati_nuovi(hourly):
        print("ℹ️ Nessun aggiornamento completo trovato per ICON-CH2. Uscita silenziosa.")
        sys.exit(0)
        
    print("ℹ️ Dati ICON-CH2 pronti e aggiornati. Generazione del grafico...")
    ...
    # Layout con 4 grafici (Zero termico sostituisce le medie e alte quote)
    ...
    # Formattazione Asse X Finale (ogni 12 ore per leggibilità sui 5 giorni)
    titolo_in_basso = "Modello MeteoSwiss ICON-CH2 Ensemble Mean (120h)   |   Data e Ora (Fuso Orario Locale)"
    ...
    if token and chat_id:
        ...
        # Testo strettamente richiesto: "ICON-CH2"
        payload = {
            "chat_id": chat_id, 
            "caption": "ICON-CH2"
        }
