...
def fetch_dati_con_retry() -> dict:
    ...
    headers = {"User-Agent": "MeteoBot-ICONCH1/2.0"}
...
def main():
    ...
    if not is_new:
        print("⏳ Nessun nuovo run completo (Media+Spread) per ICON-CH1. Uscita silenziosa.")
        sys.exit(0)
        
    utc_offset = data.get("utc_offset_seconds", 0)
    nome_run = ottieni_nome_run(ultima_ora, utc_offset, ORIZZONTE_ORE)
        
    print(f"ℹ️ Trovato nuovo run ICON-CH1 {nome_run}. Generazione del grafico...")
